...
public class BSTVisualization extends JFrame implements ActionListener, KeyListener {
	...
	private JTextField student_field;
	private int X = 300, Y = 75;
	private Graphics2D g2;
	private Rectangle size;
	private JLabel labelInorder, labelPreorder, labelPostorder, labelHeight, student_label;
	...
	private static class Node {
		...
		Node(String name, int roll) {
			...
			data = new JLabel(name + ", " + roll + "", SwingConstants.CENTER);
	...
	private void initialize() {
		...
		topPanel.setBorder(new EmptyBorder(0, 0, 50, 0));
		...
		student_label = new JLabel("Name, Roll no: ");
		student_label.setFont(new Font("Calibri", Font.BOLD, 20));
		topRightPanel.add(student_label);

		student_field = new JTextField("");
		student_field.setFont(new Font("Arial", Font.BOLD, 20));
		student_field.setPreferredSize(new Dimension(150, 30));
		student_field.addKeyListener(this);
		topRightPanel.add(student_field);
		...
		student_field.requestFocusInWindow();
	...
	public void actionPerformed(ActionEvent evt) {
		// if (tf.isEnabled()) {
		try {
			String name = student_field.getText().split(", ")[0];
			int roll = Integer.parseInt(student_field.getText().split(", ")[1]);
	...
	public void add(String name, int roll) {
		...
		if (root == null) {
			...
			while (curr != null) {
				pre = curr;
				currData = Integer.parseInt(curr.data.getText().split(", ")[1]);
				if (roll == currData) {
					JOptionPane.showMessageDialog(null, roll + " already exists.");
					return;
				} else if (currData > roll) {
					curr = curr.left;
				} else {
					curr = curr.right;
				}
			...
			currData = Integer.parseInt(pre.data.getText().split(", ")[1]);
			int x = pre.data.getX();
			int y = pre.data.getY();
			...
			if (currData > roll) {
				...
				newNode.p = new Points(x + preDimension.width, y + preDimension.height / 2, x + X + currDimension.width / 2,
						y + Y + currDimension.height / 2);
			}
		}
	...
	public void delete(int data) {
		if (root == null) {
			...
			while (curr != null) {
				int info = Integer.parseInt(curr.data.getText().split(", ")[1]);
				if (info == data) {
					break;
				} else if (info > data) {
					pre = curr;
			...
			if (curr == null) { // data is not find.
				...
				if (curr != root) {
					...
					// curr.data>pre.data
					int preData = Integer.parseInt(pre.data.getText().split(", ")[1]);
					int currData = Integer.parseInt(curr.data.getText().split(", ")[1]);
					if (currData > preData) {
						pre.right = address;
					} else {
						pre.left = address;
	...
	private String inorder(Node root) {
		...
		return inorder(root.left) + root.data.getText().split(", ")[1] + " " + inorder(root.right);
	}
	...
	public String preorder(Node root) {
		...
		return root.data.getText().split(", ")[1] + " " + preorder(root.left) + preorder(root.right);
	}
	...
	public String postorder(Node root) {
		...
		return postorder(root.left) + postorder(root.right) + root.data.getText().split(", ")[1] + " ";
	}
	...
	private void reArrangeNode(Node node, Node pre, int X) {
		...
		if (root == node) {
			...
			int preData = Integer.parseInt(pre.data.getText().split(", ")[1]);
			int nodeData = Integer.parseInt(node.data.getText().split(", ")[1]);
			if (nodeData < preData) {
				...
				node.p = new Points(x + preDimension.width, y + preDimension.height / 2, x + X + currDimension.width / 2,
						y + Y + currDimension.height / 2);
			}
		}
